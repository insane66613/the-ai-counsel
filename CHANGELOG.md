...
## [Unreleased]
### Security
- Protected all non-loopback REST and MCP access with `LLM_COUNCIL_ADMIN_TOKEN` while preserving token-free localhost/desktop use.
- Blocked conversation-ID path traversal, bounded multipart uploads before buffering, sanitized raw model HTML, and made JSON persistence crash-safe.
- Updated frontend dependencies to patched compatible releases; `npm audit` now reports zero vulnerabilities.
