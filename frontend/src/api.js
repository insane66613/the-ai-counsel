...
export const api = {
  ...
  async startOAuth(providerId) {
    const response = await apiFetch(`${API_BASE}/api/oauth/${encodeURIComponent(providerId)}/start`, {
      method: 'POST',
    });
    if (!response.ok) {
      const data = await response.json().catch(() => ({}));
  ...
  async disconnectOAuth(providerId) {
    const response = await apiFetch(`${API_BASE}/api/oauth/${encodeURIComponent(providerId)}`, {
      method: 'DELETE',
    });
    if (!response.ok) {
      const data = await response.json().catch(() => ({}));
  ...
  async disconnectAllProviders() {
    const response = await apiFetch(`${API_BASE}/api/settings/disconnect-all-providers`, {
      method: 'POST',
    });
    if (!response.ok) {
      const data = await response.json().catch(() => ({}));
  ...
  async setCredentialStorage(mode) {
    const response = await apiFetch(`${API_BASE}/api/settings/credential-storage`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
  ...
  async discoverRelayAi() {
    const response = await apiFetch(`${API_BASE}/api/credentials/import/relay-ai/discover`);
    if (!response.ok) {
  ...
  async importRelayAi(ids, replaceExisting = false) {
    const response = await apiFetch(`${API_BASE}/api/credentials/import/relay-ai`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
  ...
  async dismissRelayImport() {
    const response = await apiFetch(`${API_BASE}/api/settings/relay-ai-import-dismissed`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({}),
    });
